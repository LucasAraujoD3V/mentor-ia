...
- Logo no site: `.logo-full` (recorte por CSS da `20260922_223141000_iOS.jpg`, símbolo + nome, sem slogan/pilares) no banner do topo (`.hero`: seção preta em tela cheia, logo à direita e título/texto/botões à esquerda; no celular a logo fica em cima) e no rodapé. A foto `assets/hero.jpg` não é mais usada no topo. **Não** colocar logo no menu ao lado do nome: pedido explícito do usuário.
