...
- Logo no site: `.logo-full` (recorte por CSS da `20260922_223141000_iOS.jpg`, símbolo + nome, sem slogan/pilares) no banner do topo (`.hero`: seção preta em tela cheia, logo inteira (`.hero__logo`, a imagem original com slogan e pilares) à direita e título/texto/botões à esquerda; no celular a logo fica em cima) e no rodapé. A foto `assets/hero.jpg` não é mais usada no topo. **Não** colocar logo no menu ao lado do nome: pedido explícito do usuário.
- Cache: `index.html` carrega `styles.css?v=...` e `script.js?v=...`. **Troque o `v=` a cada mudança de CSS/JS**, senão o GitHub Pages (cache de 10 min) mostra a página nova com CSS velho (aconteceu: banner ficou branco).
...
- Links para o brandbook no site: botões na seção #mvv (abrir a página e baixar o PDF) e link no rodapé.
